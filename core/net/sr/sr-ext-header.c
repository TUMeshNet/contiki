...
/**
 * \file
 *         Management of extension headers for ContikiSR.
 *
 * \author James Novino <james.novino@temple.edu>.
 */
...
#define UIP_SRHO_BUF              ((struct uip_srho_hdr *)&uip_buf[uip_l2_l3_hdr_len])
#define UIP_SRHO_NEXT_BUF         ((struct uip_ext_hdr *)&uip_buf[uip_l2_l3_hdr_len + SR_HOP_BY_HOP_LEN])
#define UIP_RH_BUF                ((struct uip_routing_hdr *)&uip_buf[uip_l2_l3_hdr_len])
#define UIP_SR_SRH_BUF           ((struct uip_sr_srh_hdr *)&uip_buf[uip_l2_l3_hdr_len + SR_RH_LEN])
#define UIP_EXT_HDR_OPT_BUF       ((struct uip_ext_hdr_opt *)&uip_buf[uip_l2_l3_hdr_len + uip_ext_opt_offset])
#define UIP_EXT_HDR_OPT_PADN_BUF  ((struct uip_ext_hdr_opt_padn *)&uip_buf[uip_l2_l3_hdr_len + uip_ext_opt_offset])
#define UIP_EXT_HDR_OPT_SR_BUF   ((struct uip_ext_hdr_opt_sr *)&uip_buf[uip_l2_l3_hdr_len + uip_ext_opt_offset])
/*---------------------------------------------------------------------------*/
int
sr_verify_srh_header(int uip_ext_opt_offset)
{
  sr_instance_t *instance;
  int down;
  uint16_t sender_rank;
  uint8_t sender_closer;
  uip_ds6_route_t *route;
  sr_parent_t *sender = NULL;

  if(UIP_SRHO_BUF->len != ((SR_HOP_BY_HOP_LEN - 8) / 8)
      || UIP_EXT_HDR_OPT_SR_BUF->opt_type != UIP_EXT_HDR_OPT_SR
      || UIP_EXT_HDR_OPT_SR_BUF->opt_len != SR_HDR_OPT_LEN) {

    PRINTF("SR: Hop-by-hop extension header has wrong size or type (%u %u %u)\n",
        UIP_SRHO_BUF->len,
        UIP_EXT_HDR_OPT_SR_BUF->opt_type,
        UIP_EXT_HDR_OPT_SR_BUF->opt_len);
    return 0; /* Drop */
  }

  instance = sr_get_instance(UIP_EXT_HDR_OPT_SR_BUF->instance);
  if(instance == NULL) {
    PRINTF("SR: Unknown instance: %u\n",
           UIP_EXT_HDR_OPT_SR_BUF->instance);
    return 0;
  }

  if(UIP_EXT_HDR_OPT_SR_BUF->flags & SR_HDR_OPT_FWD_ERR) {
    PRINTF("SR: Forward error!\n");
    /* We should try to repair it by removing the neighbor that caused
         ...
         us. */
    if(SR_IS_STORING(instance)) {
      route = uip_ds6_route_lookup(&UIP_IP_BUF->destipaddr);
      if(route != NULL) {
        uip_ds6_route_rm(route);
      }
    }
    SR_STAT(sr_stats.forward_errors++);
    /* Trigger DAO retransmission */
    sr_reset_dio_timer(instance);
    /* drop the packet as it is not routable */
    return 0;
  }

  if(!instance->current_dag->joined) {
    PRINTF("SR: No DAG in the instance\n");
    return 0;
  }
  down = 0;
  if(UIP_EXT_HDR_OPT_SR_BUF->flags & SR_HDR_OPT_DOWN) {
    down = 1;
  }

  sender_rank = UIP_HTONS(UIP_EXT_HDR_OPT_SR_BUF->senderrank);
  sender = nbr_table_get_from_lladdr(sr_parents, packetbuf_addr(PACKETBUF_ADDR_SENDER));

  if(sender != NULL && (UIP_EXT_HDR_OPT_SR_BUF->flags & SR_HDR_OPT_RANK_ERR)) {
    /* A rank error was signalled, attempt to repair it by updating
     * the sender's rank from ext header */
    sender->rank = sender_rank;
    if(SR_IS_NON_STORING(instance)) {
      ...
      sr_select_dag(instance, sender);
    }
  }

  sender_closer = sender_rank < instance->current_dag->rank;

  PRINTF("SR: Packet going %s, sender closer %d (%d < %d)\n", down == 1 ? "down" : "up",
   sender_closer,
   sender_rank,
  ...
  if((down && !sender_closer) || (!down && sender_closer)) {
    PRINTF("SR: Loop detected - senderrank: %d my-rank: %d sender_closer: %d\n",
    ...
    if(sender != NULL) {
      ...
      sr_schedule_unicast_dio_immediately(instance);
    }
    if(UIP_EXT_HDR_OPT_SR_BUF->flags & SR_HDR_OPT_RANK_ERR) {
      SR_STAT(sr_stats.loop_errors++);
      PRINTF("SR: Rank error signalled in SR option!\n");
      /* Packet must be dropped and dio trickle timer reset, see RFC6550 - 11.2.2.2 */
      sr_reset_dio_timer(instance);
      return 0;
    }
    PRINTF("SR: Single error tolerated\n");
    SR_STAT(sr_stats.loop_warnings++);
    UIP_EXT_HDR_OPT_SR_BUF->flags |= SR_HDR_OPT_RANK_ERR;
    return 1;
  }
...
/*---------------------------------------------------------------------------*/
#if SR_WITH_NON_STORING
int
sr_srh_get_next_hop(uip_ipaddr_t *ipaddr)
{
  uint8_t *uip_next_hdr;
  int last_uip_ext_len = uip_ext_len;
  sr_dag_t *dag;
  sr_ns_node_t *dest_node;
  sr_ns_node_t *root_node;
  ...
  dag = sr_get_dag(&UIP_IP_BUF->destipaddr);
  root_node = sr_ns_get_node(dag, &dag->dag_id);
  dest_node = sr_ns_get_node(dag, &UIP_IP_BUF->destipaddr);

  if((uip_next_hdr != NULL && *uip_next_hdr == UIP_PROTO_ROUTING
      && UIP_RH_BUF->routing_type == SR_RH_TYPE_SRH) ||
     ...
     * address, via sr_process_srh_header. We turn this address into a link-local to enable
...
sr_process_srh_header(void)
{
  ...
  if(uip_next_hdr != NULL && *uip_next_hdr == UIP_PROTO_ROUTING
    ...
    cmpri = UIP_SR_SRH_BUF->cmpr >> 4;
    cmpre = UIP_SR_SRH_BUF->cmpr & 0x0f;
    padding = UIP_SR_SRH_BUF->pad >> 4;
    path_len = ((ext_len - padding - SR_RH_LEN - SR_SRH_LEN - (16 - cmpre)) / (16 - cmpri)) + 1;
    (void)path_len;

    PRINTF("SR: read SRH, path len %u, segments left %u, Cmpri %u, Cmpre %u, ext len %u (padding %u)\n",
    ...
    if(segments_left == 0) {
      ...
      uint8_t *addr_ptr = ((uint8_t *)UIP_RH_BUF) + SR_RH_LEN + SR_SRH_LEN + (i * (16 - cmpri));
...
insert_srh_header(void)
{
  ...
  uint8_t path_len;
  uint8_t ext_len;
  uint8_t cmpri, cmpre; /* ComprI and ComprE fields of the SR Source Routing Header */
  uint8_t *hop_ptr;
  uint8_t padding;
  sr_ns_node_t *dest_node;
  sr_ns_node_t *root_node;
  ...
  PRINTF("SR: SRH creating source routing header with destination ");
  PRINT6ADDR(&UIP_IP_BUF->destipaddr);
  PRINTF(" \n");
  ...
  root_node = sr_ns_get_node(dag, &dag->dag_id);
  if(root_node == NULL) {
    PRINTF("SR: SRH root node not found\n");
    return 0;
  }

  if(!sr_ns_is_node_reachable(dag, &UIP_IP_BUF->destipaddr)) {
    PRINTF("SR: SRH no path found to destination\n");
    return 0;
  }
  ...
  if(node == root_node) {
    PRINTF("SR: SRH no need to insert SRH\n");
    return 1;
  }

  while(node != NULL && node != root_node) {

    sr_ns_get_node_global_addr(&node_addr, node);
  ...
  PRINTF("SR: SRH Path len: %u, ComprI %u, ComprE %u, ext len %u (padding %u)\n",
  ...
  if(uip_len + ext_len > UIP_BUFSIZE) {
    PRINTF("SR: Packet too long: impossible to add source routing header (%u bytes)\n", ext_len);
    return 1;
  }
  ...
  /* Initialize SR Source Routing Header */
  UIP_SR_SRH_BUF->cmpr = (cmpri << 4) + cmpre;
  UIP_SR_SRH_BUF->pad = padding << 4;
...
#else /* SR_WITH_NON_STORING */
int insert_srh_header(void);
#endif /* SR_WITH_NON_STORING */
/*---------------------------------------------------------------------------*/
static int
update_srh_header(void)
{
  sr_instance_t *instance;
  ...
  if(UIP_IP_BUF->proto == UIP_PROTO_SRHO && UIP_EXT_HDR_OPT_SR_BUF->opt_type == UIP_EXT_HDR_OPT_SR) {
    if(UIP_SRHO_BUF->len != ((SR_HOP_BY_HOP_LEN - 8) / 8)
        || UIP_EXT_HDR_OPT_SR_BUF->opt_len != SR_HDR_OPT_LEN) {

      PRINTF("SR: Hop-by-hop extension header has wrong size (%u %u)\n",
          UIP_EXT_HDR_OPT_SR_BUF->opt_len,
          uip_ext_len);
      return 0; /* Drop */
    }
    ...
    if(instance == NULL || !instance->used || !instance->current_dag->joined) {
      PRINTF("SR: Unable to add/update hop-by-hop extension header: incorrect instance\n");
      uip_ext_len = last_uip_ext_len;
      return 0; /* Drop */
    }

    PRINTF("SR: Updating SR option\n");
    /* Update sender rank and instance, will update flags next */
    UIP_EXT_HDR_OPT_SR_BUF->senderrank = UIP_HTONS(instance->current_dag->rank);
    UIP_EXT_HDR_OPT_SR_BUF->instance = instance->instance_id;

    if(SR_IS_STORING(instance)) { /* In non-storing mode, downwards traffic does not have the SRH option */
      /* Check the direction of the down flag, as per Section 11.2.2.3,
            ...
            SR_HDR_OPT_FWD_ERR should be flagged. */
      if((UIP_EXT_HDR_OPT_SR_BUF->flags & SR_HDR_OPT_DOWN)) {
        if(uip_ds6_route_lookup(&UIP_IP_BUF->destipaddr) == NULL) {
          UIP_EXT_HDR_OPT_SR_BUF->flags |= SR_HDR_OPT_FWD_ERR;
          PRINTF("SR forwarding error\n");
          /* We should send back the packet to the originating parent,
                but it is not feasible yet, so we send a No-Path DAO instead */
          PRINTF("SR generate No-Path DAO\n");
          parent = sr_get_parent((uip_lladdr_t *)packetbuf_addr(PACKETBUF_ADDR_SENDER));
          if(parent != NULL) {
            dao_output_target(parent, &UIP_IP_BUF->destipaddr, SR_ZERO_LIFETIME);
          }
          /* Drop packet */
          return 0;
        }
        ...
        if(uip_ds6_route_lookup(&UIP_IP_BUF->destipaddr) == NULL) {
          /* No route was found, so this packet will go towards the SR
                root. If so, we should not set the down flag. */
          UIP_EXT_HDR_OPT_SR_BUF->flags &= ~SR_HDR_OPT_DOWN;
          PRINTF("SR option going up\n");
        } else {
          /* A DAO route was found so we set the down flag. */
          UIP_EXT_HDR_OPT_SR_BUF->flags |= SR_HDR_OPT_DOWN;
          PRINTF("SR option going down\n");
        }
      }
    }
  }
...
/*---------------------------------------------------------------------------*/
static int
insert_srh_header(const sr_instance_t *instance)
{
  int uip_ext_opt_offset;
  int last_uip_ext_len;
  uint8_t temp_len;
  ...
  PRINTF("SR: Creating hop-by-hop option\n");
  if(uip_len + SR_HOP_BY_HOP_LEN > UIP_BUFSIZE) {
    PRINTF("SR: Packet too long: impossible to add hop-by-hop option\n");
    uip_ext_len = last_uip_ext_len;
    return 0;
  }

  /* Move existing ext headers and payload UIP_EXT_BUF further */
  memmove(UIP_SRHO_NEXT_BUF, UIP_EXT_BUF, uip_len - UIP_IPH_LEN);
  memset(UIP_SRHO_BUF, 0, SR_HOP_BY_HOP_LEN);

  /* Update IP and SRH protocol and fields */
  UIP_SRHO_BUF->next = UIP_IP_BUF->proto;
  UIP_IP_BUF->proto = UIP_PROTO_SRHO;

  /* Initialize SRH option */
  UIP_SRHO_BUF->len = (SR_HOP_BY_HOP_LEN - 8) / 8;
  UIP_EXT_HDR_OPT_SR_BUF->opt_type = UIP_EXT_HDR_OPT_SR;
  UIP_EXT_HDR_OPT_SR_BUF->opt_len = SR_HDR_OPT_LEN;
  UIP_EXT_HDR_OPT_SR_BUF->flags = 0;
  UIP_EXT_HDR_OPT_SR_BUF->senderrank = UIP_HTONS(instance->current_dag->rank);
  UIP_EXT_HDR_OPT_SR_BUF->instance = instance->instance_id;
  uip_len += SR_HOP_BY_HOP_LEN;
  temp_len = UIP_IP_BUF->len[1];
  UIP_IP_BUF->len[1] += SR_HOP_BY_HOP_LEN;
  if(UIP_IP_BUF->len[1] < temp_len) {
    UIP_IP_BUF->len[0]++;
  }

  uip_ext_len = last_uip_ext_len + SR_HOP_BY_HOP_LEN;

  /* Update header before returning */
  return update_srh_header();
}
/*---------------------------------------------------------------------------*/
void
sr_remove_header(void)
{
  uint8_t temp_len;
  uint8_t sr_ext_hdr_len;
  uint8_t *uip_next_hdr;
  ...
  while(uip_next_hdr != NULL) {
    switch(*uip_next_hdr) {
      ...
      case UIP_PROTO_ROUTING:
        ...
        *uip_next_hdr = UIP_EXT_BUF->next;
        sr_ext_hdr_len = (UIP_EXT_BUF->len * 8) + 8;
        temp_len = UIP_IP_BUF->len[1];
        uip_len -= sr_ext_hdr_len;
        UIP_IP_BUF->len[1] -= sr_ext_hdr_len;
        if(UIP_IP_BUF->len[1] > temp_len) {
          UIP_IP_BUF->len[0]--;
        }
        PRINTF("SR: Removing SR extension header (type %u, len %u)\n", *uip_next_hdr, sr_ext_hdr_len);
        memmove(UIP_EXT_BUF, ((uint8_t *)UIP_EXT_BUF) + sr_ext_hdr_len, uip_len - UIP_IPH_LEN);
        break;
      case UIP_PROTO_DESTO:
        /*
...
sr_update_header(void)
{
  ...
  if(default_instance->current_dag->rank == ROOT_RANK(default_instance)) {
    /* At the root, remove headers if any, and insert SRH or SRH
     * (SRH is inserted only if the destination is in the DODAG) */
    sr_remove_header();
    if(SR_IS_NON_STORING(default_instance)) {
      return insert_srh_header();
    } else {
      return insert_srh_header(default_instance);
    }
  } else {
    if(uip_ds6_is_my_addr(&UIP_IP_BUF->srcipaddr)
        && UIP_IP_BUF->ttl == uip_ds6_if.cur_hop_limit) {
      /* Insert SRH option at source. Checking the address is not sufficient because
       * in non-storing mode, a packet may go up and then down the same path again */
      return insert_srh_header(default_instance);
    } else {
      /* Update SRH option at forwarders */
      return update_srh_header();
    }
  }
}
